...
async fn download_file() -> Result<(), Box<dyn Error>> {
    ...
    let _goto:String = String::from("goto");
    ...
    let _execute_command_result = execute_command(&_goto).await;
    ...
    //wait
    // debug!("wait 10 sec");
    // let _ = wait_seconds_of_browser(5);
...
async fn execute_command(cmd: &String) -> Result<(), Box<dyn Error>> {
    ...
    if cmd == "close" {
        ...
        let _ = wait_seconds_of_browser(_driver,5).await;
        debug!("execute_command  _cmd => {}", cmd);
        let result_close_browser = close_browser(_driver).await;
        ...
        debug!("execute_command  _cmd => {}", cmd);
        let result_close_browser = goto_webpage(_driver.clone()).await;
        let _ = match result_close_browser {
            Ok(_web_element) => {
                info!(r#"ACTION_BROWSER_CLOSE => Ok"#);
            }
...
async fn goto_webpage(_driver: WebDriver) -> Result<(), Box<dyn Error>> {
